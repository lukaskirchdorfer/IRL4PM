...
def build_decision_dataset(
    ...
    # Include all columns to allow dynamic feature functions
...
# =========================================================
# 3) Train per-agent Logistic Regression models
# =========================================================

def train_logistic_regression_per_agent(
...
) -> Tuple[Dict[Any, LogisticRegression], Dict[str, Dict[Any, int]], Dict[Any, StandardScaler]]:
    """
...
# =========================================================
# 3B) Train a single global Logistic Regression model
# =========================================================

def train_logistic_regression_global(
    train_log: pd.DataFrame,
    numerical_features: List[str],
    categorical_features: List[str],
    C: float = 1.0,
    ...
    scalers: Dict[str, Any] = None,
    standardize_features: bool = True
) -> Tuple[LogisticRegression, Dict[str, Dict[Any, int]], StandardScaler]:
    """
    Train a single LogisticRegression across all agents using dynamic features.
    Returns (model, encoders, scaler).
    """
    encoders = build_encoders(train_log, categorical_features)
    feat_fn = lambda agent, row, t, cdf: feature_dynamic(
        agent, row, t, cdf, encoders=encoders, 
    ...
    if len(np.unique(y)) < 2:
        raise ValueError("Not enough class diversity to train a global model.")
...
def evaluate_topk_decisions_per_agent(
    ...
    models: Dict[Any, "LogisticRegression"],
    encoders: Dict[str, Dict[Any, int]],
